...
const AzureAuthenticationButton = ({ onAuthenticated }: any): JSX.Element => {
  const router = useRouter();
	...
	const showLogInButton = (): any => {
		return (
			<button id="authenticationButton" onClick={() => logIn("loginPopup")} className="storybook-button--primary">
		...
		return (
			<div id="authenticationButtonDiv">
				<div id="authentication">
					<button id="authenticationButton" onClick={() => logOut()}className="storybook-button--primary">
						Log Out of Microsoft SSO
					</button>
				</div>
			</div>
		);
